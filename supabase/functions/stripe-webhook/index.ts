...
async function handleSubscription(invoice: any, supabaseUrl: string, serviceRoleKey: string) {
    ...
    const subscriptions_table = 'resume_subscriptions'
    const plans_table = 'resume_plans'
...
async function handlePayment(invoice: any, supabaseUrl: string, serviceRoleKey: string) {
    ...
    const planResponse = await fetch(`${supabaseUrl}/rest/v1/resume_plans?price_id=eq.${priceId}&select=plan_type,monthly_limit`, {
        headers: {
            'Authorization': `Bearer ${serviceRoleKey}`,
            'apikey': serviceRoleKey
        }
